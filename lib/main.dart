...
class _VibeState extends State<VibeApp>{
 ...
 return MaterialApp(debugShowCheckedModeBanner:false,title:'vibe<3',themeMode:light?ThemeMode.light:ThemeMode.dark,darkTheme:themeData(true),theme:themeData(false),home:logged?Shell(name:name,username:username,light:light,onTheme:theme,onName:rename,onProfile:updateProfile,onLogout:logout):AuthScreen(onLogin:saveUser));}
}
...
class Settings extends StatelessWidget{
 final String name,username;final bool light;final ValueChanged<bool> onTheme;final ValueChanged<String> onName;final Future<void> Function(String,String) onProfile;final VoidCallback onLogout;
 const Settings({super.key,required this.name,required this.username,required this.light,required this.onTheme,required this.onName,required this.onProfile,required this.onLogout});
 @override Widget build(BuildContext c)=>ListView(padding:const EdgeInsets.all(18),children:[
  const Text('Settings',style:TextStyle(fontSize:28,fontWeight:FontWeight.w900)),const SizedBox(height:18),
  Card(color:Theme.of(c).brightness==Brightness.dark?surface:Colors.white,child:ListTile(contentPadding:const EdgeInsets.all(12),leading:Avatar(name:name,size:56),title:Text(name,style:const TextStyle(fontWeight:FontWeight.w800)),subtitle:Text('@$username',style:const TextStyle(color:muted)),onTap:()=>edit(c))),
  const SizedBox(height:10),
  Card(color:Theme.of(c).brightness==Brightness.dark?surface:Colors.white,child:SwitchListTile(value:!light,onChanged:(v)=>onTheme(!v),secondary:const Icon(Icons.dark_mode_outlined),title:const Text('Dark Y2K theme'),subtitle:const Text('Purple / blue / pink',style:TextStyle(color:muted)))),
  section(c,'Privacy & security',[row(Icons.lock_outline,'Privacy','Last seen and read receipts'),row(Icons.shield_outlined,'Security','Server authentication')]),
  section(c,'Notifications',[row(Icons.notifications_none,'Notifications','Messages and calls')]),
  section(c,'About',[row(Icons.info_outline,'About vibe<3','Version 1.0.0')]),const SizedBox(height:14),
  OutlinedButton.icon(onPressed:onLogout,icon:const Icon(Icons.logout),label:const Text('Sign out'))
 ]);
 Widget section(BuildContext c,String t,List<Widget> x)=>Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Padding(padding:const EdgeInsets.fromLTRB(4,18,4,7),child:Text(t.toUpperCase(),style:const TextStyle(color:muted,fontSize:11,fontWeight:FontWeight.w800,letterSpacing:1.1))),Card(color:Theme.of(c).brightness==Brightness.dark?surface:Colors.white,child:Column(children:x))]);
 Widget row(IconData i,String t,String s)=>ListTile(leading:Icon(i),title:Text(t,style:const TextStyle(fontWeight:FontWeight.w700)),subtitle:Text(s,style:const TextStyle(color:muted,fontSize:12)),trailing:const Icon(Icons.chevron_right));
 void edit(BuildContext c){
  final n=TextEditingController(text:name),u=TextEditingController(text:username);bool busy=false;
  showDialog(context:c,builder:(_)=>StatefulBuilder(builder:(ctx,setLocal)=>AlertDialog(
   title:const Text('Edit profile'),
   content:Column(mainAxisSize:MainAxisSize.min,children:[
    TextField(controller:n,decoration:const InputDecoration(labelText:'Name')),
    const SizedBox(height:12),
    TextField(controller:u,decoration:const InputDecoration(labelText:'Username',prefixText:'@')),
    const SizedBox(height:8),
    const Text('3–30 chars: a-z, 0-9, _',style:TextStyle(color:muted,fontSize:12))
   ]),
   actions:[
    TextButton(onPressed:busy?null:()=>Navigator.pop(ctx),child:const Text('Cancel')),
    FilledButton(onPressed:busy?null:()async{
      final nn=n.text.trim(),uu=u.text.trim().toLowerCase().replaceFirst('@','');
      if(nn.length<2||!RegExp(r'^[a-z0-9_]{3,30}$').hasMatch(uu)){ScaffoldMessenger.of(c).showSnackBar(const SnackBar(content:Text('Проверь имя и юзернейм')));return;}
      setLocal(()=>busy=true);
      try{await onProfile(nn,uu);if(ctx.mounted)Navigator.pop(ctx);}catch(e){
        final s=e.toString();final msg=s.contains('username_taken')?'Этот юзернейм уже занят':s.contains('invalid_username')?'Неверный юзернейм':'Не удалось сохранить профиль';
        if(c.mounted)ScaffoldMessenger.of(c).showSnackBar(SnackBar(content:Text(msg)));
